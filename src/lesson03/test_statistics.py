...
class TestNumpyStatistics(unittest.TestCase):
    ...
    def test_statistics_std(self):
        ...
        np.testing.assert_array_almost_equal(np.std(a, axis=1), [0.816497, 0.816497, 0.816497])  # standard deviation by each row
        np.testing.assert_array_almost_equal(np.std(a, axis=0), [2.44949, 2.44949, 2.44949])  # standard deviation by each column
